#!/usr/bin/env node
/**
 * Smoke test: 模拟"用 Use this template 创建新仓库"的用户完整启动路径，验证模板真的可用。
 *
 * 步骤（在 /tmp 里的干净副本执行，不影响 demo）：
 *   1. 拷贝当前 repo（不含 node_modules / dist / .git）
 *   2. 符号链 node_modules（避免重装依赖，约 60s → 1s）
 *   3. pnpm init:group "Smoke Test Lab"     ← 模拟 template 用户第一步
...
// 传 --github / --site-url 避免 init 留下 placeholder（否则 verify 不会挂，
// 但 grep 还能扫到 YOUR_GITHUB_OWNER 等字样，smoke 要测完整 happy path）
const res2 = run(
  `node scripts/init-group.mjs "${GROUP_NAME}" ` +
  `--github=smoke-lab/smoke-wiki --site-url=https://smoke.pages.dev`
);
...
if (warningCount > 0) {
  ...
  fail(
    `Template 用户的冷启动路径有 ${warningCount} 个 broken reference。`,
    '这意味着 init:group 后留下了 dangling link — 模板对 template 用户不完整可用。'
  );
}
