...
| **关联 session** | [W19 共读](/sessions/2026-w19-deepseek-r1/) |
| **带读人** | [@phd-senior-1](/members/phd-senior-1/) |
| **状态** | ✅ published（W19 共读后由带读人定稿） |
...
> 这一节由带读人 [@phd-senior-1](/members/phd-senior-1/) 写。读一篇论文之前先想清楚"为什么现在读它，对组里有什么影响"。

我们组 [Test-time Reasoning 主线](/themes/test-time-reasoning/) 的核心问题是 **"大模型在测试时多花算力，能不能真正提升 reasoning 能力（而非简单的 verbosity）"**。
...
- **方法论**：R1-Zero 的"涌现"现象说明 base model 可能本来就有 reasoning 能力，只是缺一个"激活"的 RL 信号 —— 这跟我们 Postdoc-1 之前在 [MoE 训练稳定性主线](/themes/moe-sparsity/) 看到的"专家激活的稀疏性是预先存在的"是同一个故事
...
[@postdoc-1](/members/postdoc-1/) 提出一个值得验证的猜想：**G 应该自适应于 reward 方差** —— 简单 prompt（reward 几乎都对或都错）G = 4 就够，难 prompt G = 32 才能拿到有效梯度。这个组里没看到现成工作，可以做。
...
### 1. 短期接入 baseline（W21 截止）

我们组的 SFT-30B 模型马上接 GRPO，验证三件事：

- **GRPO infra 端到端能跑**（@phd-senior-1）
- **AIME 提升幅度**：是否能复现"几百步训练涨 30+ 个点"
- **G 自适应**：postdoc-1 的猜想做 ablation（G = 4/8/16/32 + reward variance buckets）

详细 plan 见 W19 session 的 action items。
...
2. **G = 16 的选择有没有更深层依据？** —— 论文没有 ablation，组里 [@phd-senior-1](/members/phd-senior-1/) 在 W19 提出做这个 ablation
...
- **G 自适应**（[@postdoc-1](/members/postdoc-1/) 提出，**值得做 short paper**）
- **reasoning 蒸馏的 lower bound**：R1 → 7B 还有效吗？800k 数据 → 80k / 8k 还行吗？（[@phd-mid-1](/members/phd-mid-1/) 感兴趣）
- **多模态版 R1**：visual / video 任务的 verifiable reward 怎么设计？这是我们组下一篇 paper 的**潜在方向**。

## 共读历史

- **2026-W19**（[2026-05-11](/sessions/2026-w19-deepseek-r1/)）—— 首次共读，[@phd-senior-1](/members/phd-senior-1/) 带读，重点 §3 GRPO + §4 涌现行为

> 后续重读会自动更新到这里（计划由一个 agent skill 维护）。
...
- [W18 · DeepSeek-V4 长上下文](/sessions/2026-w18-deepseek-v4/) —— 同公司，对照看训练栈演化
- [Test-time Reasoning 主线](/themes/test-time-reasoning/) —— 组内立场
